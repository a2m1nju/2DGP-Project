...
announcement_x = 0
announcement_text = "이번역은 슬픔역입니다. 내리실 문은 왼쪽입니다."
...
def init():
    ...
    try:
        announcement_sound = load_wav('./음악/슬픔역.wav')
        announcement_sound.set_volume(70)
    except:
        print("안내 방송 파일을 찾을 수 없습니다.")
        announcement_sound = None

    bgm = load_music('./음악/스테이지2.wav')
    bgm.set_volume(50)
    bgm.repeat_play()
...
def update():
    ...
    if boss_spawned and boss and boss.hp <= 0:
        if not is_clearing:
            is_clearing = True
    ...
    if current_time - spawn_timer > spawn_cooldown:
        ...
        if server.enemies_killed_count >= max_spawn_count:
            ...
            e = Zombie(girl, type=random.randint(1, 4))
            ...
            game_world.add_collision_pair('lightning:enemy', None, e)
...
def draw():
    ...
    if not is_clearing:
        ...
        font.draw(50, 550, f'KILLS: {server.enemies_killed_count} / 21', (255, 255, 255))
